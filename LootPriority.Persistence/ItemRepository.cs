        ...
        public Item GetItem(int id)
        {
            ...
            using (SqlConnection connection = new SqlConnection(SqlHelper.connectionString))
            {
                string query =
                    ...
                    "i2.ID QuestRewardID " +
                    "FROM Item i " +
                    "LEFT JOIN Slot s ON i.SlotID = s.ID " +
                    "LEFT JOIN ItemClass ic ON ic.ItemID = i.ID " +
                    "LEFT JOIN Class c ON c.ID = ic.ClassID " +
                    "LEFT JOIN Item i2 ON i.ID = i2.RewardFromQuestItem " +
                    "WHERE i.ID = @id OR i.RewardFromQuestItem = @id " +
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        {
                            if (items[itemId].QuestRewards.Select(qr => qr.Id).Contains(questRewardId))
                            {
                                var characterClass = reader.GetEnum<CharacterClass>(5);
                                if (!items[itemId].Classes.Contains(characterClass)) items[itemId].Classes.Add(characterClass);
                            }
                            else items[itemId].QuestRewards.Add(items[questRewardId]);
                        }
                        else
                        {
